...
See [config.md](config.md) for the full reference — every key, what it does, accepted
key names, and how to tune movement feel.
...
- Marity intercepts arrow keys and the two click keys **only while enabled**, and only
  ...
  Shift/Ctrl keys are never touched at all.
- It uses a low-level global keyboard hook (`WH_KEYBOARD_LL`), which does not require
  ...
  processes from sending focus-affecting input to windows running elevated ("as
...
- Only one instance runs at a time (enforced via a named mutex); launching a second
  copy just shows a message box and exits.
